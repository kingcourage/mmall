...
public class CloseOrderTask {
    ...
    public void closeOrderTashV3(){
        ...
        if(setnxResult != null && setnxResult.intValue() == 1){
            ...
            if(lockValueStr != null && System.currentTimeMillis() > Long.parseLong(lockValueStr)){
                ...
                //当key没有旧值得我时候，即key不存在，返回nil->获取锁
    ...
    @Scheduled(cron = "0 */1 * * * ?") //每分钟执行
    public void closeOrderTashV4(){
        RLock lock = redissonManager.getRedisson().getLock(Const.REDIS_LOCK.CLOSE_ORDER_TASK_LOCK);
        boolean getLock = false;
        try {
           if(getLock = lock.tryLock(2,5, TimeUnit.SECONDS)){
               log.info("Redisson获取到 分布式锁:{}，ThreadName:{}",Const.REDIS_LOCK.CLOSE_ORDER_TASK_LOCK,Thread.currentThread().getName());
               int hour = Integer.parseInt(PropertiesUtil.getProperty("close.order.task.time.hour","2"));
               iOrderService.closeOrder(hour);
           }else{
